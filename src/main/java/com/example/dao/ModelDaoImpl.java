...
import com.example.model.dto.FlatQueryConditionDTO;
import com.example.model.dto.Page;
import com.example.model.dto.QueryCondition;
import com.example.model.dto.SingleModelConditionDTO;
...
import com.example.toolkit.ReflectionKit;
...
public class ModelDaoImpl<T extends AbstractModelEntity> implements BaseModelDao<T> {
    ...
    public int insert(T entity) {
        ...
        Result<List<Map<String, Object>>> result = modelDataService.write(Collections.singletonList(entity));
        Optional<Map<String, Object>> any = result.getData().stream().findAny();
        if (any.isPresent()) {
    ...
    public int delete(Wrapper<T> wrapper) {
        ...
        List<Long> ids = ts.stream().map(AbstractModelEntity::getId).collect(Collectors.toList());
        if (CollectionUtils.isNotEmpty(ids)) {
            modelDataService.deleteById(modelLabel, ids);
        }
        return ids.size();
    ...
    public int saveOrUpdateBatch(Collection<T> entityList) {
        ...
        Result<List<Map<String, Object>>> saveResult = modelDataService.write(entityList);
        List<Map<String, Object>> data = saveResult.getData();
