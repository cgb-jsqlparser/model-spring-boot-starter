...
public final class ReflectionKit {
     ...
     * @return Class
     */
    public static Class<?> getInterfaceClassGenericType(final Class<?> clazz, final int index) {
        Type[] genericInterfaces = clazz.getGenericInterfaces();
        Type genType = genericInterfaces[index];
        if (!(genType instanceof ParameterizedType)) {
            log.warn(String.format("Warn: %s's superclass not ParameterizedType", clazz.getSimpleName()));
            return Object.class;
        }
    ...
    public static Class<?> getFieldGenericType(final Field field, final int index) {
        ...
        if (!(genType instanceof ParameterizedType)) {
            log.warn(String.format("Warn: %s not ParameterizedType", field.getName()));
            return Object.class;
        }
        Type[] params = ((ParameterizedType) genType).getActualTypeArguments();
        if (index >= params.length || index < 0) {
            log.warn(String.format("Warn: Index: %s, Size of %s's Parameterized Type: %s .", index,
                    field.getName(), params.length));
            return Object.class;
        }
        if (!(params[index] instanceof Class)) {
            log.warn(String.format("Warn: %s not set the actual class on field generic parameter",
                    field.getName()));
            return Object.class;
        }
        return (Class<?>) params[index];
